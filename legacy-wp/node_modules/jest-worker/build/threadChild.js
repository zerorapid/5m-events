...
_nodeWorker_threads().parentPort.on('message', messageListener);
function reportMemoryUsage() {
  ...
  _nodeWorker_threads().parentPort.postMessage(msg);
}
function reportSuccess(result) {
  ...
  try {
    _nodeWorker_threads().parentPort.postMessage([_types.PARENT_MESSAGE_OK, result]);
  } catch (error) {
    ...
    if ((0, _isDataCloneError.isDataCloneError)(error)) {
      try {
        _nodeWorker_threads().parentPort.postMessage([_types.PARENT_MESSAGE_OK, (0, _safeMessageTransferring.packMessage)(result)]);
        return;
...
function reportError(error, type) {
  ...
  _nodeWorker_threads().parentPort.postMessage([type, error.constructor && error.constructor.name, error.message, error.stack, typeof error === 'object' ? {
    ...error
  } : error]);
}
function end() {
