...
const formatTitle = (title, row, rowIndex) => row.reduce((formattedTitle, value) => {
  ...
  if (placeholder === JSON_PLACEHOLDER) return interpolateJsonPlaceholder(formattedTitle, normalisedValue);
...
const interpolatePrettyPlaceholder = (title, value) => {
  const prettyValue = (0, _prettyFormat().format)(value, {
    maxDepth: 1,
    min: true
  });
  return title.replace(PRETTY_PLACEHOLDER, () => prettyValue);
};
const interpolateJsonPlaceholder = (title, value) => {
...
function stringifyJson(value) {
  ...
  return `${JSON.stringify(value, (_, entry) => typeof entry === 'bigint' ? `${entry}n` : entry)}`;
}
function isCyclic(value, ancestors = []) {
