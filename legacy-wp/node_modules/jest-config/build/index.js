...
isProjectOptions = false) {
  ...
  } = await (0, _normalize.default)(initialOptions, argv, configPath, projectIndex, isProjectOptions);
  const {
    globalConfig,
    projectConfig
...
async function readConfigs(argv, projectPaths) {
  ...
  if (projects.length > 0) {
    ...
    const parsedConfigs = await Promise.all(projects.filter(root => {
      ...
      const parentConfigDirname = configPath ? path().dirname(configPath) : cwd;
      ...
      const suppliesGlobalConfig = globalConfig == null && projectIndex === 0 || configPath != null && typeof root === 'string' && (0, _resolveConfigPath.default)(root, cwd, projectIsCwd) === configPath;
      return readConfig(argv, root, skipArgvConfigOption, parentConfigDirname, projectIndex,
      // we wanna skip the warning if this is the "main" project
      projectIsCwd, skipArgvConfigOption && !suppliesGlobalConfig);
    }));
...
async function normalize(initialOptions, argv, configPath, projectIndex = Number.POSITIVE_INFINITY, isProjectOptions) {
  ...
  const {
    ...
    ...(isProjectOptions ? [] : ['coverageThreshold']), 'globals', 'moduleNameMapper', 'testEnvironmentOptions', 'transform'],
    ...(isProjectOptions && {
  ...
  if (options.resolver) {
    ...
    await (0, _jestResolve().preloadResolver)(newOptions.resolver);
  }
