...
var _testExecutionContext = __webpack_require__("./src/testExecutionContext.ts");
...
const eventHandler = (event, state) => {
  switch (event.name) {
    ...
    case 'error':
      {
        ...
        // from.
        const context = (0, _testExecutionContext.getTestExecutionContext)();
        if (event.promise) {
          ...
          if (context?.test) {
            ...
            const hookTarget = state.unhandledRejectionErrorByPromiseByHook.get(context.hook);
            if (hookTarget) {
              target = hookTarget;
            } else {
              target = new Map();
              state.unhandledRejectionErrorByPromiseByHook.set(context.hook, target);
            }
          }
          target.set(event.promise, event.error);
          state.unhandledRejectionErrorByPromiseTarget.set(event.promise, target);
          const hasOwner = context?.test !== undefined || context?.hook !== undefined;
          if ((0, _testExecutionContext.hasActiveDescribeRetryAttempt)() && !hasOwner) {
    ...
    case 'error_handled':
      {
        const target = state.unhandledRejectionErrorByPromiseTarget.get(event.promise);
    ...
    event.test.errors = event.test.errors.map(errors => {
      ...
      if (Array.isArray(errors)) {
        ...
        if (originalError == null) {
          ...
          error = new Error(originalError.message || `thrown: ${(0, _prettyFormat.format)(originalError, {
            maxDepth: 3
          })}`);
        }
      } else {
...
function isAssertionError(error) {
  return error && (error instanceof _nodeAssert.AssertionError || error.name === _nodeAssert.AssertionError.name || error.code === 'ERR_ASSERTION');
}
...
const injectGlobalErrorHandlers = parentProcess => {
  ...
  const uncaughtException = [...parentProcess.rawListeners('uncaughtException')];
  const unhandledRejection = [...parentProcess.rawListeners('unhandledRejection')];
  const rejectionHandled = [...parentProcess.rawListeners('rejectionHandled')];
...
const run = async () => {
  ...
  _expect.jestExpect.setState({
    currentConcurrentTestName: () => {
      const test = (0, _testExecutionContext.getTestExecutionContext)()?.test;
      return test ? (0, _utils.getTestID)(test) : undefined;
    },
    currentTestIdentity: () => {
      return (0, _testExecutionContext.getTestExecutionContext)()?.test;
    }
  });
...
const _runTestsForDescribeBlock = async (describeBlock, runContext) => {
  const state = (0, _state.getState)();
  ...
  let numRetriesAvailable = Number.parseInt(String(retryOptions.numRetries), 10) || 0;
  ...
  while (true) {
    ...
    const retryAttempt = {
      errorsBeforeAttempt: new Map(tests.map(test => [test, test.errors.length])),
      expectStateBeforeAttempt: {
        currentTestName,
        testFailing
      },
      hasExternalExpectState: expectedAssertionsNumber !== null || isExpectingAssertions || suppressedErrors.length > 0,
    ...
    (0, _testExecutionContext.startDescribeRetryAttempt)();
    try {
      ...
      (0, _testExecutionContext.finishDescribeRetryAttempt)();
    }
    const hasErrors = hasErrorsAddedDuringAttempt(retryAttempt);
    const nonRetryable = retryAttempt.hasExternalExpectState || state.processErrorGeneration > retryAttempt.processErrorGenerationBeforeAttempt || state.unhandledErrors.length > retryAttempt.unhandledErrorsBeforeAttempt;
    ...
    const snapshotState = _expect.jestExpect.getState().snapshotState;
    if (snapshotState) {
      for (const test of tests) {
        snapshotState.clear(test);
      }
    }
    _expect.jestExpect.setState(retryAttempt.expectStateBeforeAttempt);
    await (0, _state.dispatch)({
      describeBlock,
      name: 'describe_retry'
    });
    if ((retryOptions.waitBeforeRetry ?? 0) > 0) {
      await new Promise(resolve => setTimeout(resolve, retryOptions.waitBeforeRetry));
    }
    numRetriesAvailable--;
  }
};
const clearTestErrors = (retryAttempt, logErrorsBeforeRetry) => {
...
const hasErrorsAddedDuringAttempt = retryAttempt => {
  for (const [test, errorStart] of retryAttempt.errorsBeforeAttempt) {
...
const getTestsForDescribeBlock = describeBlock => {
  const tests = [];
...
const takeNewSuppressedErrors = suppressedErrorsBeforeHook => {
  const {
    suppressedErrors
  } = _expect.jestExpect.getState();
  const newSuppressedErrors = suppressedErrors.slice(suppressedErrorsBeforeHook);
...
const _runTestsForDescribeBlockOnce = async (describeBlock, runContext) => {
  await (0, _state.dispatch)({
    describeBlock,
  ...
  if (!isSkipped) {
    for (const hook of beforeAll) {
      ...
      const suppressedErrorsBeforeHook = trackSuppressedErrors ? _expect.jestExpect.getState().suppressedErrors.length : undefined;
      await _callCircusHook({
        describeBlock,
        hook
      });
      if (suppressedErrorsBeforeHook !== undefined) {
        for (const error of takeNewSuppressedErrors(suppressedErrorsBeforeHook)) {
          (0, _utils.addErrorToEachTestUnderDescribe)(describeBlock, error);
        }
      }
    }
  }
  ...
  if (runContext.rng && !runContext.shuffledDescribeBlocks.has(describeBlock)) {
    describeBlock.children = (0, _shuffleArray.default)(describeBlock.children, runContext.rng);
  ...
  if (!isSkipped) {
    for (const hook of afterAll) {
      ...
      if (suppressedErrorsBeforeHook !== undefined) {
        for (const error of takeNewSuppressedErrors(suppressedErrorsBeforeHook)) {
          (0, _state.getState)().unhandledErrors.push(error);
        }
      }
    }
  }
...
const _runTest = async (test, parentSkipped) => (0, _testExecutionContext.runInTestExecutionContext)({
  test
}, () => _runTestInContext(test, parentSkipped));
const _runTestInContext = async (test, parentSkipped) => {
  await (0, _state.dispatch)({
    name: 'test_start',
    test
  });
...
const _callCircusHook = async ({
  ...
  describeBlock,
  testContext = {}
}) => (0, _testExecutionContext.runInTestExecutionContext)({
  describeBlock,
  hook,
  test
...
const setState = state => {
  ...
  (0, _jestUtil.setGlobal)(globalThis, _types.STATE_SYM, initializedState);
  (0, _jestUtil.protectProperties)(initializedState, ['describeRetryOptions', 'hasFocusedTests', 'hasStarted', 'includeTestLocationInResult', 'maxConcurrency', 'processErrorGeneration', 'seed', 'testNamePattern', 'testTimeout', 'unhandledErrors', 'unhandledRejectionErrorByPromise', 'unhandledRejectionErrorByPromiseByHook', 'unhandledRejectionErrorByPromiseTarget']);
...
const testCaseReportHandler = (testPath, sendMessageToJest, formatRetryError) => event => {
  switch (event.name) {
    case 'test_started':
      {
    ...
    case 'test_done':
      {
        ...
        const testCaseResult = (0, _utils.parseSingleTestResult)(testResult, formatRetryError);
...
exports.startDescribeRetryAttempt = exports.runInTestExecutionContext = exports.hasActiveDescribeRetryAttempt = exports.getTestExecutionContext = exports.finishDescribeRetryAttempt = void 0;
...
const testExecutionContext = new _nodeAsync_hooks.AsyncLocalStorage();
let activeDescribeRetryAttempts = 0;
const getTestExecutionContext = () => testExecutionContext.getStore();
exports.getTestExecutionContext = getTestExecutionContext;
const runInTestExecutionContext = (context, callback) => testExecutionContext.run({
  ...testExecutionContext.getStore(),
  ...context
}, callback);
exports.runInTestExecutionContext = runInTestExecutionContext;
const startDescribeRetryAttempt = () => {
  activeDescribeRetryAttempts++;
};
exports.startDescribeRetryAttempt = startDescribeRetryAttempt;
const finishDescribeRetryAttempt = () => {
  activeDescribeRetryAttempts--;
};
exports.finishDescribeRetryAttempt = finishDescribeRetryAttempt;
const hasActiveDescribeRetryAttempt = () => activeDescribeRetryAttempts > 0;
exports.hasActiveDescribeRetryAttempt = hasActiveDescribeRetryAttempt;
...
exports.WAIT_BEFORE_RETRY = exports.TEST_TIMEOUT_SYMBOL = exports.STATE_SYM = exports.RETRY_TIMES_SETTER = exports.RETRY_TIMES = exports.RETRY_IMMEDIATELY = exports.LOG_ERRORS_BEFORE_RETRY = exports.EVENT_HANDLERS = void 0;
var Symbol = globalThis['jest-symbol-do-not-touch'] || globalThis.Symbol;
/**
 * Copyright (c) Meta Platforms, Inc. and affiliates.
 *
...
const RETRY_TIMES_SETTER = exports.RETRY_TIMES_SETTER = Symbol.for('RETRY_TIMES_SETTER');
...
const takeUnhandledRejectionErrors = (state, errorsByPromise) => {
...
const getHooksForDescribeBlock = describeBlock => {
...
const addPendingHookRejections = (state, hook) => {
  const errorsByPromise = state.unhandledRejectionErrorByPromiseByHook.get(hook);
  if (!errorsByPromise) {
    return;
  }
  ...
  for (const error of errors) {
    if (hook.type === 'beforeAll') {
      (0, _utils.addErrorToEachTestUnderDescribe)(hook.parent, error, hook.asyncError);
    } else if (hook.type === 'afterAll') {
      state.unhandledErrors.push([error, hook.asyncError]);
    }
  }
};
const addPendingTestRejections = (state, test) => {
  for (const error of takeUnhandledRejectionErrors(state, test.unhandledRejectionErrorByPromise)) {
...
const unhandledRejectionHandler = (runtime, waitForUnhandledRejections) => {
  return async (event, state) => {
    if (event.name === 'hook_start') {
      ...
      const errorsByPromise = state.unhandledRejectionErrorByPromiseByHook.get(hook) ?? new Map();
      const errors = takeUnhandledRejectionErrors(state, errorsByPromise);
      state.unhandledRejectionErrorByPromiseByHook.delete(hook);
      if (type === 'beforeAll') {
        (0, _jestUtil.invariant)(describeBlock, 'always present for `*All` hooks');
        ...
        // too complicated, so we'll consider them to be global.
        for (const error of errors) {
          const hookError = [error, asyncError];
      ...
      (0, _jestUtil.invariant)(test, 'always present for `*Each` hooks');
      for (const error of takeUnhandledRejectionErrors(state, test.unhandledRejectionErrorByPromise)) {
        const testError = [error, event.test.asyncError];
      ...
      state.unhandledErrors.push(...takeUnhandledRejectionErrors(state, state.unhandledRejectionErrorByPromise));
    }
  };
};
...
const callAsyncCircusFn = (testOrHook, testContext, {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isGeneratorFunction(fn)) {
      returnedValue = _co.default.wrap(fn).call(testContext);
    } else {
      try {
...
const makeRunResult = (describeBlock, unhandledErrors) => {
  ...
  return {
    ...
    unhandledErrors: unhandledErrorsDetailed.map(_jestMessageUtil.flattenErrorStack),
...
const makeSingleTestResult = test => {
  ...
  return {
    ...
    errors: errorsDetailed.map(_jestMessageUtil.flattenErrorStack),
    errorsDetailed,
    failing: test.failing,
    invocations: test.invocations,
    location,
    numPassingAsserts: test.numPassingAsserts,
    retryReasons: retryReasonsDetailed.map(_jestMessageUtil.flattenErrorStack),
...
const _getError = errors => {
  ...
  if (asyncError) {
    const message = `thrown: ${(0, _prettyFormat.format)(error, {
      maxDepth: 3
    })}`;
  ...
  return new Error(`thrown: ${(0, _prettyFormat.format)(error, {
    maxDepth: 3
  })}`);
};
...
const parseSingleTestResult = (testResult, formatRetryError) => {
  let status;
  ...
  return {
    ...
    retryMessages: formatRetryError ? testResult.retryReasonsDetailed.map(formatRetryError) : undefined,
...
exports.runAndTransformResultsToJestFormat = exports.initialize = exports.eventHandler = exports.collectTestsWithoutRunning = void 0;
var _expect = require("@jest/expect");
...
const makeRetryErrorFormatter = (config, globalConfig, testPath) => error => (0, _jestMessageUtil.formatErrorStack)(error, config, globalConfig, testPath);
const initialize = async ({
  ...
  (0, _state.getState)().expand = globalConfig.expand;
  ...
  environment.global[_types.RETRY_TIMES_SETTER] = retryOptions => {
    const state = (0, _state.getState)();
    if (state.hasStarted) {
      throw new Error('Cannot set retry options after tests have started running. Retry options must be set synchronously.');
    }
  ...
  if (sendMessageToJest) {
    (0, _state.addEventHandler)((0, _testCaseReportHandler.default)(testPath, sendMessageToJest, makeRetryErrorFormatter(config, globalConfig, testPath)));
  }
...
const collectTestsWithoutRunning = async ({
  config,
  testPath
}) => {
  const {
  ...
  const walk = (block, parent) => {
    for (const child of block.children) {
      ...
      const deselected = testNamePattern != null && !testNamePattern.test((0, _utils.getTestID)(child));
      const skipped = parent.skipped || child.mode === 'skip' || hasFocusedTests && child.mode === undefined || deselected;
      let status;
      let wouldRun;
      if (skipped) {
        status = 'pending';
  ...
  return (0, _testResult.makeCollectedTestResult)(assertionResults, {
    displayName: config.displayName,
    testFilePath: testPath
  });
};
exports.collectTestsWithoutRunning = collectTestsWithoutRunning;
const runAndTransformResultsToJestFormat = async ({
  ...
  const formatRetryError = makeRetryErrorFormatter(config, globalConfig, testPath);
  ...
  const assertionResults = runResult.testResults.map(testResult => {
    ...
    const {
      startedAt,
      ...assertionResult
    } = (0, _utils.parseSingleTestResult)(testResult, formatRetryError);
  ...
  if (runResult.unhandledErrors.length > 0) {
    ...
    failureMessage = `${failureMessage || ''}\n\n${runResult.unhandledErrorsDetailed.map(error => (0, _jestMessageUtil.formatExecError)(error, config, globalConfig)).join('\n')}`;
  }
  await (0, _state.dispatch)({
    name: 'teardown'
  });
