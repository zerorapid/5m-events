...
class ScriptTransformer {
  ...
  _buildCacheKeyFromFileInfo(fileData, filename, transformOptions, transformerCacheKey) {
    if (transformerCacheKey != null) {
      return (0, _nodeCrypto().createHash)('sha1').update(transformerCacheKey).update('\0', 'utf8').update(callerSupport(transformOptions)).update('\0', 'utf8').update(CACHE_VERSION).digest('hex').slice(0, 32);
    }
    return (0, _nodeCrypto().createHash)('sha1').update(fileData).update('\0', 'utf8').update(transformOptions.configString).update('\0', 'utf8').update(transformOptions.instrument ? 'instrument' : '').update('\0', 'utf8').update(callerSupport(transformOptions)).update('\0', 'utf8').update(filename).update('\0', 'utf8').update(CACHE_VERSION).digest('hex').slice(0, 32);
  }
...
function getScriptCacheKey(filename, instrument, options) {
  const mtime = fs().statSync(filename).mtime;
  return [filename, mtime.getTime().toString(), instrument ? 'instrumented' : '', callerSupport(options)].filter(Boolean).join('_');
}
