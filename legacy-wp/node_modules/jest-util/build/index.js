...
function emitAccessWarning(obj, key) {
  ...
  process.emitWarning(`'${propertyName}' property was accessed on [${objName}] after it was soft deleted`, {
    code: 'JEST-01',
    detail: ['Jest deletes objects that were set on the global scope between test files to reduce memory leaks.', 'Currently it only "soft" deletes them and emits this warning if those objects were accessed after their deletion.', 'In future versions of Jest, this behavior will change to "on", which will likely fail tests.', 'Set `testEnvironmentOptions.globalsCleanup` to "on" to delete them now and reduce memory usage,', 'or to "off" to disable the cleanup and this warning.', 'See https://jestjs.io/docs/configuration#testenvironmentoptions-object'].map(s => `  ${s}`).join('\n'),
...
const isError = exports.isError = typeof Error.isError === 'function' ? Error.isError : _types().isNativeError;
