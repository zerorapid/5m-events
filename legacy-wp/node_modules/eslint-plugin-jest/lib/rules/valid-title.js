      ...
      CallExpression(node) {
        ...
        if (ignoreSpaces === false && title.trim().length !== title.length) {
          context.report({
            ...
            node: argument,
            fix: fixer => [fixer.replaceText(argument, quoteStringValue(argument).replace(/^([`'"]) +?/u, '$1').replace(/ +?([`'"])$/u, '$1'))]
          });
        }
        ...
        if (firstWord.toLowerCase() === unprefixedName) {
          context.report({
            ...
            node: argument,
            fix: fixer => [fixer.replaceText(argument, quoteStringValue(argument).replace(/^([`'"]).+? /u, '$1'))]
          });
        }
