...
function _expectUtils() {
  const data = require("@jest/expect-utils");
  _expectUtils = function () {
    return data;
  };
  return data;
}
...
class ModuleMocker {
  ...
  // is reset.
  _resetWhenCalledWithSubMocks(f) {
  ...
  _makeWhenDispatcherImpl(f) {
    const ensureConfig = mock => this._ensureMockConfig(mock);
    ...
    return function whenCalledWithDispatcherImpl(...callArgs) {
      const config = ensureConfig(f);
      const matching = config.whenCalledWithRegistrations.filter(branch => (0, _expectUtils().equals)(branch.matchers, callArgs, [_expectUtils().iterableEquality]));
      // (1) Forward find: first matching branch with a queued once.
      const onceBranch = matching.find(branch => ensureConfig(branch.subMock).specificMockImpls.length > 0);
      if (onceBranch) {
    ...
    if (metadata.type === 'object') {
      ...
      f.getMockImplementation = () => {
        ...
        if (mockConfig.mockImpl === dispatcherImpl) {
          return mockConfig.fallbackImpl;
        }
      ...
      f.whenCalledWith = (...args) => {
        ...
        const subMock = this._makeComponent({
          type: 'function'
        });
        mockConfig.whenCalledWithRegistrations.push({
          matchers: args,
          subMock
        });
        return subMock;
