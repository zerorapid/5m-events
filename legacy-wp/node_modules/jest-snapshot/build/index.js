...
function semverGte(version, other) {
  ...
  if (prettierPath && !prettier) {
    try {
      ...
      if (semverGte(prettier.version, '3.0.0')) {
        const {
          createSyncFn
        } = require(require.resolve('synckit', {
          [Symbol.for('jest-resolve-outside-vm-option')]: true
        }));
        workerFn = createSyncFn(require.resolve(/*webpackIgnore: true*/'./worker'));
...
class SnapshotState {
  ...
  markSnapshotsAsCheckedForTest(testName) {
    for (const uncheckedKey of this._uncheckedKeys) {
      const keyTestName = (0, _snapshotUtils.keyToTestName)(uncheckedKey);
  ...
  _markKeyChecked(key, testIdentity) {
  ...
  _bumpCounter(testName, testIdentity) {
  ...
  clear(testIdentity) {
    ...
    this._inlineSnapshots = this._inlineSnapshots.filter(snapshot => !record.inlineSnapshots.has(snapshot));
    ...
    if (writes !== undefined && this._writeCount - writes.writeCountBefore === writes.ownWrites) {
      this._dirty = writes.dirtyBefore;
      this._writeCount = writes.writeCountBefore;
    }
  }
  _incrementSnapshotCount(status, testIdentity) {
    this[status]++;
    const record = this._recordFor(testIdentity);
    if (record !== undefined) {
  ...
  fail(testName, _received, key, testIdentity) {
    const count = this._bumpCounter(testName, testIdentity);
    if (!key) {
      key = (0, _snapshotUtils.testNameToKey)(testName, count);
    }
...
const _toMatchSnapshot = config => {
  ...
  if (typeof properties === 'object') {
    ...
    if (propertyPass) {
      ...
      const key = snapshotState.fail(fullTestName, received, undefined, testIdentity);
...
const _toThrowErrorMatchingSnapshot = (config, fromPromise) => {
  ...
  while ('cause' in error) {
    error = error.cause;
    if ((0, _jestUtil.isError)(error) || error instanceof Error) {
      message += `\nCause: ${error.message}`;
    } else {
